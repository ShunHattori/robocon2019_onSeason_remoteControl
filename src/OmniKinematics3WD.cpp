...
void OmniKinematics3WD::getOutput(int x, int y, int yaw, float yawAngle, int pwm[3])
{
    ...
    { //ユーザーからの回転命令があるとき
        ...
        pwm[1] = -XVector * cos((yawAngle + 120) * DEG_TO_RAD) - YVector * sin((yawAngle + 120) * DEG_TO_RAD) + YawVector;
        pwm[2] = +XVector * cos((yawAngle + 60) * DEG_TO_RAD) + YVector * sin((yawAngle + 60) * DEG_TO_RAD) + YawVector;
    }
    else
    { //IMUがYAW軸を自動補正するとき
        ...
        pwm[1] = -XVector * cos((yawAngle - 120) * DEG_TO_RAD) - YVector * sin((yawAngle - 120) * DEG_TO_RAD) + outputYawPWM;
        pwm[2] = +XVector * cos((yawAngle - 60) * DEG_TO_RAD) + YVector * sin((yawAngle - 60) * DEG_TO_RAD) + outputYawPWM;
    }
